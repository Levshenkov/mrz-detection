...
function getNumberToLetterHeightRatio(card) {
  ...
  card = filterMAD(card, 2.5, card => card.image.height + Math.random() * 0.01)
  const grouped = _.groupBy(card, c => {
  ...
  const metric = mean(grouped.number.map(c => c.image.height)) / mean(grouped.letter.map(c => c.image.height))
